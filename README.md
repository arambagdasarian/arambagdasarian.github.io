...
- The adaptation-networks paper uses a minimal derivative of `trajectory_comparison.mp4`, rendered from the same full-sample network data. It preserves the four observed periods, node identities, edges, and smooth transitions, with seven selected labels and a monochrome palette. The optimized video is `assets/video/trajectory-comparison.mp4`; its still is `assets/images/trajectory-network.webp`. `visual.style: "network"` provides a slightly wider preview for label legibility. Clicking opens the video because this entry has no public paper URL. The original two-panel source is preserved outside this public repository.
